...
if __name__ == '__main__':
    ...
    logger.info("  num_steps: %d", scheduler.num_steps)
    logger.info("  beta range: [%.6f, %.6f]", scheduler.betas.min(), scheduler.betas.max())
    logger.info("  alpha_bar range: [%.6f, %.6f]", scheduler.alphas_cumprod.min(), scheduler.alphas_cumprod.max())
    ...
    logger.info("Forward diffusion test:")
    logger.info("  x_0 std: %.4f", x_0.std())
    logger.info("  x_t[t=0] std: %.4f  (should be ~same as x_0)", x_t[0].std())
    logger.info("  x_t[t=999] std: %.4f  (should be ~1.0, pure noise)", x_t[1].std())
