...
class StructureEvaluator:
    ...
    def evaluate_structures(self, generated_dir: Path) -> dict:
        ...
        if not xyz_files:
            logger.warning("No XYZ files found in %s", generated_dir)
            return {}
        
        logger.info("Evaluating %d structures with comprehensive metrics...", len(xyz_files))
        ...
        for xyz_file in xyz_files:
            try:
                ...
                logger.error("Error processing %s: %s", xyz_file.name, e)
    ...
    def print_comparison(self, generated_results: dict):
        ...
        if 1.29 <= gen_mean <= 1.49:
            bond_range_msg = "  ✓ Within physical range [1.29, 1.49] Å"
        else:
            bond_range_msg = "  ✗ Outside physical range [1.29, 1.49] Å"

        frac_valid = generated_results['connectivity']['fraction_valid']
        if frac_valid >= 0.95:
            conn_msg = "  ✓ Excellent connectivity"
        elif frac_valid >= 0.80:
            conn_msg = "  ⚠ Good connectivity"
        else:
            conn_msg = "  ✗ Poor connectivity"
        ...
        lines = [
            ...
            "=" * 80,
            "",
            "Number of structures: %d" % generated_results['num_structures'],
            "",
            "-" * 80,
            "BOND LENGTH STATISTICS (Å):",
            "  Reference:  %.4f ± %.4f" % (self.reference_stats['bond_lengths']['mean'], self.reference_stats['bond_lengths']['std']),
            "  Generated:  %.4f ± %.4f" % (generated_results['bond_lengths']['mean'], generated_results['bond_lengths']['std']),
            "  Generated Median: %.4f" % generated_results['bond_lengths']['median'],
            "  Generated IQR:    [%.4f, %.4f]" % (generated_results['bond_lengths']['q25'], generated_results['bond_lengths']['q75']),
            "  Wasserstein Distance: %.6f" % generated_results['wasserstein_distances']['bond_lengths'],
            bond_range_msg,
            "",
            "-" * 80,
            "RADIUS OF GYRATION (Å):",
            "  Reference:  %.4f ± %.4f" % (self.reference_stats['radii']['mean'], self.reference_stats['radii']['std']),
            "  Generated:  %.4f ± %.4f" % (generated_results['radii']['mean'], generated_results['radii']['std']),
            "  Generated Median: %.4f" % generated_results['radii']['median'],
            "  Wasserstein Distance: %.6f" % generated_results['wasserstein_distances']['radii'],
            "",
            "-" * 80,
            "SHAPE DESCRIPTORS:",
            "  Asphericity:",
            "    Reference:  %.4f ± %.4f" % (self.reference_stats['asphericities']['mean'], self.reference_stats['asphericities']['std']),
            "    Generated:  %.4f ± %.4f" % (generated_results['asphericities']['mean'], generated_results['asphericities']['std']),
            "    Wasserstein Distance: %.6f" % generated_results['wasserstein_distances']['asphericities'],
            "  Acylindricity:    %.4f ± %.4f" % (generated_results['acylindricities']['mean'], generated_results['acylindricities']['std']),
            "  Shape Anisotropy: %.4f ± %.4f" % (generated_results['shape_anisotropies']['mean'], generated_results['shape_anisotropies']['std']),
            "",
            "  Gyration Tensor Eigenvalues:",
            "    λ₁: %.4f ± %.4f" % (generated_results['eigenvalues']['lambda1']['mean'], generated_results['eigenvalues']['lambda1']['std']),
            "    λ₂: %.4f ± %.4f" % (generated_results['eigenvalues']['lambda2']['mean'], generated_results['eigenvalues']['lambda2']['std']),
            "    λ₃: %.4f ± %.4f" % (generated_results['eigenvalues']['lambda3']['mean'], generated_results['eigenvalues']['lambda3']['std']),
            "",
            "-" * 80,
            "CONNECTIVITY:",
            "  Fraction valid (all degree=3): %.2f%%" % (frac_valid * 100),
            conn_msg,
            "",
            "-" * 80,
            "HARMONIC BOND ENERGY (kcal/mol):",
            "  Mean:   %.2f ± %.2f" % (generated_results['energies']['mean'], generated_results['energies']['std']),
            "  Range:  [%.2f, %.2f]" % (generated_results['energies']['min'], generated_results['energies']['max']),
            "",
            "=" * 80,
        ]
        logger.info("\n".join(lines))
    ...
    def plot_comparison(self, generated_results: dict, output_path: Path):
        ...
        logger.info("Saved publication-quality plot: %s", output_path)
        plt.close()
    ...
    def save_detailed_report(self, generated_results: dict, output_path: Path):
        ...
        logger.info("Saved detailed report: %s", output_path)
...
def main():
    ...
    if not generated_dir.exists():
        logger.error("Error: %s does not exist", generated_dir)
        return
    
    if not reference_csv.exists():
        logger.error("Error: %s does not exist", reference_csv)
        return
    
    logger.info("%s", "=" * 80)
    logger.info("COMPREHENSIVE FULLERENE EVALUATION")
    logger.info("%s", "=" * 80)
    ...
    logger.info("  - Plot saved to: %s", output_plot)
