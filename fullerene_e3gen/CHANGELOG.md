...
### 🚀 Large Fullerene Generation (C20–C720)

Breaks the C100 barrier — generates valid structures for **any even C from 20 to 720**.

#### Added

- **ContinuousCEmbedding (Plan B)** — Fourier features + MLP replaces discrete
  `nn.Embedding(720, 64)`, enabling smooth generalization to unseen C values.
  Pre-fit migration from old checkpoints via MSE regression (2000 steps).
- **GraphCoarsenLayer / GraphUncoarsenLayer (Plan D)** — Hierarchical message
  passing for graphs with N > 80 atoms. Greedy edge-contraction coarsening to
  ~N/2 nodes, 2 extra EGNN layers on coarse graph, gated uncoarsening.
- **Convex-hull dual topology** (`synthetic_topology.py`) — Deterministic
  3-regular planar graph generation for any even C ≥ 20. Uses Fibonacci
  sphere points → ConvexHull → dual graph construction.
- **Adaptive relax_steps** — Auto-scales post-generation relaxation with C
  (formula: `max(100, 50 + C*2.0)`). C60→170, C120→290, C720→1490.

#### Changed

- Sampling corrections now **ON by default**: `--sampling_project_radius`,
  `--sampling_rescale_each_step`, `--rescale_to_unit_radius`, `--filter_invalid`,
  `--ddim`, `--ddim_steps 200`, `--relax_steps -1` (auto).
- Template graph fallback chain: dataset XYZ → topology GNN → convex-hull
  dual → random 3-regular search.
- Model version bumped to v3 (checkpoint key migration handled automatically).

#### Validated Results (10 samples each, all 100%)

| C    | bond_mean | radius_mean |
|------|-----------|-------------|
| C60  | 0.4195    | 0.9663      |
| C80  | 0.3746    | 0.9465      |
| C120 | 0.3001    | 0.9764      |
