...
def save_template(pos: torch.Tensor, edges: torch.Tensor, name: str, output_dir: Path):
    ...
    logger.info("Saved %s template: %d atoms, %d edges", name, pos.size(0), edges.size(1))
...
def main():
    ...
    logger.info("Generating standard fullerene templates...")
    logger.info("=" * 60)
    ...
    logger.info("Templates saved to: %s", output_dir)
    logger.info("Usage in generate.py:")
    logger.info("  template_path = Path('templates/C60_icosahedral.pt')")
    logger.info("  template = torch.load(template_path)")
