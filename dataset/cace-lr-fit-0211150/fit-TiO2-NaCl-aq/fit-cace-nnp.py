...
collection = cace.tasks.get_dataset_from_xyz(train_path='../TiO2-electrolyte/train.xyz',
                                 valid_fraction=0.1,
                                 seed=1,
                                 cutoff=cutoff,
                                 data_key={'energy': 'energy', 'forces':'forces'}, 
                                 atomic_energies= {1: 253.24201633557695, 8: -973.8377733105831, 11: -892.7725103980401, 17: -681.9870638767183, 22: -497.5330291088536}# avg
                                 )
batch_size = 4
...
task.save_model('TiO2-NaCl-water-model.pth')
cace_nnp.to(device)
...
logging.info("training")
task.fit(train_loader, valid_loader, epochs=50, screen_nan=False)


task.save_model('TiO2-NaCl-water-model-2.pth')
cace_nnp.to(device)
...
task.save_model('TiO2-NaCl-water-model-3.pth')
