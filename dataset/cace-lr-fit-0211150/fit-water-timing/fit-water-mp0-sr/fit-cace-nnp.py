...
import sys
sys.path.append('../cace/') #torchscript branch
...
# , NeuralNetworkPotentialNoForces
...
#best model scripting
logging.info("Loading the saved model from 'best_model.pth'")
loaded_model = torch.load('best_model.pth', map_location=device)
try:
    logging.info("Scripting the bestmodel")
    scripted_model = torch.jit.script(loaded_model)
    logging.info("Successfully scripted bestmodel")
    scripted_model.save("./water-model-scripted_best.pt")
    logging.info("Model successfully saved as 'water-model-scripted_best.pt'")
