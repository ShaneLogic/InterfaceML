...
def topology_loss(
    ...
    *,
    ...
    # BCE on upper triangle with pos_weight to handle class imbalance
    # Fullerenes are 3-regular: only 3*n/2 positive edges out of n*(n-1)/2 upper-tri entries
    n = adj.size(0)
    mask = torch.triu(torch.ones((n, n), device=adj.device, dtype=torch.bool), diagonal=1)
    target = adj[mask]
    num_pos = target.sum().clamp(min=1.0)
    num_neg = (target.numel() - num_pos).clamp(min=1.0)
    pos_weight = (num_neg / num_pos).clamp(max=50.0)  # cap to avoid extreme weights
    bce = F.binary_cross_entropy_with_logits(
        logits[mask], target, pos_weight=pos_weight
    )

    # Degree regularization — normalized by target degree to keep scale manageable
    prob = torch.sigmoid(logits)
    # Zero out diagonal contribution (self-loops impossible)
    prob = prob * (1.0 - torch.eye(n, device=prob.device))
    degree = prob.sum(dim=1)
    # Normalize: relative squared error per node, so this stays O(1) regardless of n
    degree_loss = (((degree - 3.0) / 3.0) ** 2).mean()
...
class TopologyGNN(nn.Module):
    ...
    def _decode(self, z: torch.Tensor, C_value: int, num_nodes: int) -> torch.Tensor:
        ...
        h = self.dec_lin2(h)
        # Scale by 1/sqrt(hidden_dim) to prevent dot-product explosion
        logits = (h @ h.t()) / (self.hidden_dim ** 0.5)
