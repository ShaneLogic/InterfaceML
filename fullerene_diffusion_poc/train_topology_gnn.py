...
def train_topology_gnn(config: dict, device: torch.device) -> Path:
    ...
    for epoch in range(1, epochs + 1):
        ...
        print(f"[TopologyGNN] Epoch {epoch}/{epochs}  train_loss={total_loss/len(train_graphs):.4f}  val_loss={val_loss:.4f}")

    print(f"[TopologyGNN] Training complete. Best val loss: {best_val:.6f}")
    return ckpt_path
